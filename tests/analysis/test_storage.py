...
from analysis.storage import read_s3_gz_lines, read_s3_targz_lines
...
def test_read_s3_targz_lines(bucket: Bucket) -> None:
    key = 'example.tar.gz'
    bucket.upload_file(f'resources/{key}', key)
...
def test_read_s3_gz_lines(bucket: Bucket) -> None:
    key = 'example.csv.gz'
    bucket.upload_file(f'resources/{key}', key)
    results = read_s3_gz_lines(bucket, key, delimiter=',', skip_header=True)
    expected = [(1, 'cat'), (2, 'dog')]
    assert [(int(fields[0]), fields[1]) for fields in results] == expected
